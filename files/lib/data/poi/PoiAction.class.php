...
class PoiAction extends AbstractDatabaseObjectAction implements IGroupedUserListAction {
	...
	public function create() {
		...
		// trigger publication
		if (!$poi->isDisabled) {
			$action = new PoiAction([$poiEditor], 'triggerPublication');
	...
	public function update() {
		...
		foreach ($this->getObjects() as $poi) {
			...
			// add log entry
			PoiModificationLogHandler::getInstance()->edit($poi->getDecoratedObject(), (isset($this->parameters['reason']) ? $this->parameters['reason'] : ''));
			...
			// use message for various other data (MEDIUMTEXT)
			$message = isset($this->parameters['data']['message']) ? $this->parameters['data']['message'] : $poi->message;
			$teaser = isset($this->parameters['data']['teaser']) ? $this->parameters['data']['teaser'] : $poi->teaser;
			$location = isset($this->parameters['data']['location']) ? $this->parameters['data']['location'] : $poi->location;
			
			if (!empty($location)) $message .= ' ' . str_replace (',', ' ', $location);
			if (!empty($teaser)) $message .= ' ' . $teaser;
			if (!empty($optionSearch)) $message .= ' ' . $optionSearch;
			...
			SearchIndexManager::getInstance()->set(
				...
				$poi->poiID,
				$message,
				isset($this->parameters['data']['subject']) ? $this->parameters['data']['subject'] : $poi->subject,
				$poi->time,
				$poi->userID,
				$poi->username,
				$poi->languageID
	...
	public function enable() {
		...
		// publish pois
		$poiAction = new PoiAction($this->objects, 'triggerPublication');
	...
	public function prepareVisit() {
		...
		else $visitTime = date('r', $visitTime);
